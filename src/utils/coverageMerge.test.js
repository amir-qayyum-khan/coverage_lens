...
describe('coverageMerge', () => {
    ...
    test('recomputeSummaryFromMergedFiles', () => {
        const merged = [
            { lineCoverage: 50, coveredLines: 5, totalLines: 10, statementCoverage: 50, coveredStatements: 2, totalStatements: 4 },
            { lineCoverage: null, coveredLines: null, totalLines: null, statementCoverage: null, coveredStatements: null, totalStatements: null }
        ];
        const summary = recomputeSummaryFromMergedFiles(merged);
        expect(summary.lines.total).toBe(10);
        expect(summary.lines.covered).toBe(5);
